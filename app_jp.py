...
DB_PATH = Path("virtual_trader_jp.db")
...
def get_quote(symbol: str):
    ...
    try:
        ...
        quote_type = info.get("quoteType") or info.get("typeDisp") or "Japan Equity/ETF"
...
def main():
    st.set_page_config(page_title="日本股票虚拟交易系统", layout="wide")
    init_db()

    st.title("日本股票虚拟交易系统")
    st.caption("日元虚拟本金交易，行情数据来自 Yahoo Finance；不连接券商，不会动真钱。")
    ...
    with left:
        ...
        if selected_watch == "手动输入":
            symbol = st.text_input("日本股票/ETF 代码", value="7203", help="例如：7203、6758、9984、8306、6861、1306 或 7203.T")
        else:
            symbol = selected_watch.split()[0]
            st.text_input("日本股票/ETF 代码", value=symbol, disabled=True)
    ...
    if trade_mode == "按金额买入":
        ...
        if quote:
            ...
            st.caption(f"日本版按 1 股整数交易，金额取整后约剩余 {money(unused_cash)}。")
        else:
            qty = 0
